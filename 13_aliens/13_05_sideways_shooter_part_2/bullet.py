...
class Bullet(Sprite):
    ...
    def __init__(self, ai_game, offset=11):
        ...
        self.rect2.midbottom = ai_game.ship.rect.midbottom
        self.rect2.x += offset
        self.rect2.y -= offset
    ...
    def update(self):
        ...
        self.x1 += self.settings.bullet_speed
        self.x2 += self.settings.bullet_speed

        if abs(self.y1 - self.y2) > self.settings.merge_threshold:  # Smooth merging condition
            # Lasers are moving inward toward each other
            self.y1 += self.settings.bullet_merge_speed
            self.y2 -= self.settings.bullet_merge_speed
        else:
            # Once close enough, merge into a single beam
            self.rect1.height = self.settings.bullet_height * 2
            self.rect1.y = (self.y1 + self.y2) / 2 - self.rect1.height / 2
        ...
        if self.rect2:  # Only update rect2 if it exists
            ...
            # After merging, ensure the beam moves only horizontally
            self.rect1.y = self.y1
